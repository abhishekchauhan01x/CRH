...
const userSchema = new mongoose.Schema({
    ...
    // Email is optional and not unique to allow phone-first accounts
    ...
    // Phone is not unique to allow multiple accounts per number (distinguished by name)
    phone: {type:String, default: '0000000000'},
})

// Ensure a non-unique index on email (override any prior unique index)
try {
...
// Attempt to remove legacy unique email index if it exists (safe to ignore errors)
try {
    // Mongoose buffers this until connection is open
    userModel.collection.dropIndex('email_1').catch(() => {})
} catch {}

// Log the model configuration
console.log(`📋 User Model Configured:`);
...
console.log(`   Schema Fields: ${Object.keys(userModel.schema.paths).join(', ')}`);

// Add logging for when users are saved
userSchema.post('save', function(doc) {
    ...
    console.log(`   🏠 Address: ${doc.address?.line1 || 'No address'}`);
});
